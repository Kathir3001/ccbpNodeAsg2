...
const getFollowingPeopleIdsOfUser = async(username) => {
  const getTheFollowingPeopleQuery = `
   ...
   WHERE user.username = '${username}';`;
    const followingPeople = await db.all(getThe getTheFollowingPeopleQuery);
...
const tweetAccessVerification = async(request, response, next) => {
    ...
    *
    ...
    WHERE tweet.tweet_id = '${tweetId}' AND followers_user_id = '${userId}';`;
...
app.get(
    ...
    async(request, response) => {
        ...
        (SELECT COUNT() FROM like tweet_id = '${tweetId}') AS likes,
        (SELECT COUNT() FROM reply tweet_id = '${tweetId}') AS replies,
...
app.get(
    ...
    async (request, response) => {
        const { tweetId} = request.params;
        const getRepliedQuery = `SELECT name replay
        FROM user INNER JOIN replay ON user.user_id = reply.user_id
        WHERE tweet_id = '${tweetId}';
        `;
...
app.get("/user/tweets/", authentication, async(request, response) => {
    ...
    const getTweetsQuery = `
        ...
        date_time AS dateTime
        FROM tweet LEFT JOIN reply ON tweet_id = reply.tweet_id
